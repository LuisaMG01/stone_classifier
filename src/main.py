...
model, history = train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=20, device=device)

visualize_results(history)

val_acc = evaluate_model(model, dataloaders['val'], dataset_sizes['val'], class_names, device=device)
...
if val_acc < 0.9:
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=3)
    model, history = train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, num_epochs=25, device=device)
    visualize_results(history)
    val_acc_eff = evaluate_model(model, dataloaders['val'], dataset_sizes['val'], class_names, device=device)
    if val_acc_eff > val_acc:
        torch.save(model.state_dict(), 'rock_classification_model.pth')
        print(f"EfficientNet achieved better accuracy ({val_acc_eff:.4f}) and has been saved.")
    else:
